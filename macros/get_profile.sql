...
{% set column_groupings= (num_columns / 10 )|round|int%}

{% set ns=namespace(columns=[], column_set_interval=0) %}

{% for r in range(column_groupings+1)%}
    {% set column_set = columns[ns.column_set_interval: ns.column_set_interval+10] %}
    {% if column_set | length > 0 %}
        
        {% call statement('profile', fetch_result=True)%}
          with column_profiles as (
            {% for col in column_set %}
              {% set column_name=col.name %}
              select 
    ...
    {% set results=load_result('profile') %}
    {% set profile_data=results['data'] %}
    {% set ns.columns = ns.columns + profile_data %}

    {% set ns.column_set_interval=ns.column_set_interval+10 %}
